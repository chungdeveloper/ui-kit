...
public final class TrpCalendar implements Serializable, Comparable<TrpCalendar> {
     ...
     * 获取完整的农历日期
     */
    private TrpCalendar lunarCalendar;
    ...
    public TrpCalendar(String dateFormatted, String format) {
        try {
            Date date = stringToDate(dateFormatted, format);
            setTime(date);
    ...
    public TrpCalendar(int year, int month, int day) {
        this.year = year;
        this.month = month;
        this.day = day;
    }
    ...
    public TrpCalendar getLunarCalendar() {
        return lunarCalendar;
    }

    public void setLunarCalendar(TrpCalendar lunarCakendar) {
     ...
     * @return 是否是相同月份
     */
    public boolean isSameMonth(TrpCalendar calendar) {
     ...
     * @return -1 0 1
     */
    public int compareTo(TrpCalendar calendar) {
        if (calendar == null) {
            return 1;
        }
     ...
     * @return 运算差距多少天
     */
    public final int differ(TrpCalendar calendar) {
    ...
    public String getDateFromFormat(String format) {
        return getStringDateTime(format, getCalendar().getTime());
    }

    public TrpCalendar setTime(long timeMillis) {
        Calendar calendar = Calendar.getInstance();
    ...
    public static String getStringDateTime(String format, Date date) {
        android.text.format.DateFormat df = new android.text.format.DateFormat();
        return df.format(format, date).toString();
    }

    public static Date stringToDate(final String dateTime, final String format) throws ParseException {
        final SimpleDateFormat simpleDateFormat = new SimpleDateFormat(format);
        return simpleDateFormat.parse(dateTime);
    }

    public TrpCalendar setTime(Date date) {
    ...
    public boolean equals(Object o) {
        if (o != null && o instanceof TrpCalendar) {
            if (((TrpCalendar) o).getYear() == year && ((TrpCalendar) o).getMonth() == month && ((TrpCalendar) o).getDay() == day)
                return true;
        }
...
//    }
    ...
    final void mergeScheme(TrpCalendar calendar, String defaultScheme) {
        if (calendar == null)
            return;
