...
public class CalendarLayout extends LinearLayout {
     ...
     * 星期栏
     */
    com.chungld.uipack.calendar.WeekBar mWeekBar;

    /**
     * 自定义ViewPager，月视图
     */
    com.chungld.uipack.calendar.MonthViewPager mMonthView;
     ...
     * 自定义的周视图
     */
    com.chungld.uipack.calendar.WeekViewPager mWeekPager;

    /**
     * 年视图
     */
    com.chungld.uipack.calendar.YearViewPager mYearView;
...
//    private static final int GESTURE_MODE_ONLY_CALENDAR = 1;
     ...
     * @param cur 当前日期时间
     */
    private void initCalendarPosition(TrpCalendar cur) {
        int diff = com.chungld.uipack.calendar.CalendarUtil.getMonthViewStartDiff(cur, mDelegate.getWeekStart());
    ...
    void updateContentViewTranslateY() {
        TrpCalendar calendar = mDelegate.mIndexCalendar;
        if (mDelegate.getMonthViewShowMode() == com.chungld.uipack.calendar.CalendarViewDelegate.MODE_ALL_MONTH) {
            mContentViewTranslateY = 5 * mItemHeight;
        } else {
            mContentViewTranslateY = com.chungld.uipack.calendar.CalendarUtil.getMonthViewHeight(calendar.getYear(),
    ...
    final void updateCalendarItemHeight() {
        ...
        if (mContentView == null)
            return;
        TrpCalendar calendar = mDelegate.mIndexCalendar;
        updateSelectWeek(com.chungld.uipack.calendar.CalendarUtil.getWeekFromDayInMonth(calendar, mDelegate.getWeekStart()));
        if (mDelegate.getMonthViewShowMode() == com.chungld.uipack.calendar.CalendarViewDelegate.MODE_ALL_MONTH) {
            mContentViewTranslateY = 5 * mItemHeight;
        } else {
            mContentViewTranslateY = com.chungld.uipack.calendar.CalendarUtil.getMonthViewHeight(calendar.getYear(), calendar.getMonth(),
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (dy > 0 && mContentView.getTranslationY() == 0 && y >= com.chungld.uipack.calendar.CalendarUtil.dipToPx(getContext(), 98)) {
                    return false;
                }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        int weekBarHeight = com.chungld.uipack.calendar.CalendarUtil.dipToPx(getContext(), 1)
                + mDelegate.getWeekBarHeight();

        int monthHeight = com.chungld.uipack.calendar.CalendarUtil.getMonthViewHeight(year, month,
