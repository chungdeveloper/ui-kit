...
import static com.chungld.uipack.calendar.CalendarUtil.getWeekFromDayInMonth;

/**
 * 范围选择月视图
 * Created by ChungLD on 2018/9/11.
 */
public abstract class RangeMonthView extends com.chungld.uipack.calendar.BaseMonthView {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        mItemHeight = getCellHeight(mItemWidth);
        onPreviewHook();
        int count = mLineCount * 7;
        int d = 0;
        for (int i = 0; i < mLineCount; i++) {
            for (int j = 0; j < 7; j++) {
                TrpCalendar calendar = mItems.get(d);
                if (mDelegate.getMonthViewShowMode() == com.chungld.uipack.calendar.CalendarViewDelegate.MODE_ONLY_CURRENT_MONTH) {
                ...
                } else if (mDelegate.getMonthViewShowMode() == com.chungld.uipack.calendar.CalendarViewDelegate.MODE_FIT_MONTH) {
                    if (d >= count) {
                        return;
                    }
                }
     ...
     * @param j        j
     */
    private void draw(Canvas canvas, TrpCalendar calendar, int i, int j) {
        ...
        onDrawCell(canvas, calendar, x, y, hasScheme, isSelected);
    }
    ...
    public void onClick(View v) {
        ...
        if (mDelegate.mSelectedStartRangeCalendar != null && mDelegate.mSelectedEndRangeCalendar == null) {
            int minDiffer = com.chungld.uipack.calendar.CalendarUtil.differ(calendar, mDelegate.mSelectedStartRangeCalendar);
            if (minDiffer >= 0 && mDelegate.getMinSelectRange() != -1 && mDelegate.getMinSelectRange() > minDiffer + 1) {
                ...
                return;
                    ...
                    com.chungld.uipack.calendar.CalendarUtil.differ(calendar, mDelegate.mSelectedStartRangeCalendar) + 1) {
                if (mDelegate.mCalendarRangeSelectListener != null) {
        ...
        if (mParentLayout != null) {
            if (calendar.isCurrentMonth()) {
                ...
                mParentLayout.updateSelectWeek(getWeekFromDayInMonth(calendar, mDelegate.getWeekStart()));
            }
        }
        if (mDelegate.mCalendarRangeSelectListener != null) {
    ...
    protected final boolean isSelectPreCalendar(TrpCalendar calendar) {
        TrpCalendar preCalendar = com.chungld.uipack.calendar.CalendarUtil.getPreCalendar(calendar);
    ...
    protected final boolean isSelectNextCalendar(TrpCalendar calendar) {
        TrpCalendar nextCalendar = com.chungld.uipack.calendar.CalendarUtil.getNextCalendar(calendar);
     ...
     */
    protected abstract boolean onDrawSelected(Canvas canvas, TrpCalendar calendar, int x, int y, boolean hasScheme,
     ...
     */
    protected abstract void onDrawScheme(Canvas canvas, TrpCalendar calendar, int x, int y, boolean isSelected);
     ...
     * @param isSelected 是否选中
     */
    protected abstract void onDrawText(Canvas canvas, TrpCalendar calendar, int x, int y, boolean hasScheme, boolean isSelected);

    protected abstract void onDrawCell(Canvas canvas, TrpCalendar calendar, int x, int y, boolean hasScheme, boolean isSelected);
}
