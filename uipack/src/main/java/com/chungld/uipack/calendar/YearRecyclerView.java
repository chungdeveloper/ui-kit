...
public final class YearRecyclerView extends RecyclerView {
        ...
        mAdapter.setOnItemClickListener(new BaseRecyclerAdapter.OnItemClickListener() {
            ...
            public void onItemClick(int position, long itemId) {
                if (mListener != null && mDelegate != null) {
                    com.chungld.uipack.calendar.Month month = mAdapter.getItem(position);
                    if (month == null) {
                        return;
                    }
                    if (!com.chungld.uipack.calendar.CalendarUtil.isMonthInRange(month.getYear(), month.getMonth(),
    ...
    final void init(int year) {
        ...
        for (int i = 1; i <= 12; i++) {
            date.set(year, i - 1, 1);
            int mDaysCount = com.chungld.uipack.calendar.CalendarUtil.getMonthDaysCount(year, i);
            com.chungld.uipack.calendar.Month month = new com.chungld.uipack.calendar.Month();
            month.setDiff(com.chungld.uipack.calendar.CalendarUtil.getMonthViewStartDiff(year, i, mDelegate.getWeekStart()));
    ...
    final void updateWeekStart() {
        for (com.chungld.uipack.calendar.Month month : mAdapter.getItems()) {
            month.setDiff(com.chungld.uipack.calendar.CalendarUtil.getMonthViewStartDiff(month.getYear(), month.getMonth(), mDelegate.getWeekStart()));
        }
    }
    ...
    final void updateStyle(){
        for (int i = 0; i < getChildCount(); i++) {
            com.chungld.uipack.calendar.YearView view = (com.chungld.uipack.calendar.YearView) getChildAt(i);
            view.updateStyle();
            view.invalidate();
        }
    }
