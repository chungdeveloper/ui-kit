...
public class CalendarVerticalView extends FrameLayout {
     ...
     * 自定义自适应高度的ViewPager
     */
    private com.chungld.uipack.calendar.MonthViewList mMonthPager;
...
//    private YearViewPager mYearViewPager;
    ...
    public CalendarVerticalView(@NonNull Context context) {
        this(context, null);
    }

    public CalendarVerticalView(@NonNull Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
    ...
    private void init(Context context) {
        LayoutInflater.from(context).inflate(R.layout.cv_layout_calendar_vertical_view, this, true);
        ...
        mDelegate.mInnerListener = new CalendarView.OnInnerDateSelectedListener() {
            @Override
            public void onMonthDateSelected(TrpCalendar calendar, boolean isClick) {
//                calendar.getYear() == mDelegate.getCurrentDay().getYear() &&
//                        calendar.getMonth() == mDelegate.getCurrentDay().getMonth() &&
//                        calendar.getDay() < mDelegate.getCurrentDay().getDay()
...
//                mWeekPager.updateSingleSelect();
        ...
        mMonthPager.setDelegate(mDelegate);
    ...
    public void setRange(int minYear, int minYearMonth, int minYearDay,
        ...
        updateRange(minYear, minYearMonth, minYearDay, maxYear, maxYearMonth, maxYearDay);
    }

    public void setRange(Date minDate, Date maxDate) {
        TrpCalendar startDate = new TrpCalendar().setTime(minDate);
        TrpCalendar endDate = new TrpCalendar().setTime(maxDate);
        if (minDate.compareTo(maxDate) > 0) return;
        updateRange(startDate.getYear(), startDate.getMonth(), startDate.getDay(),
                endDate.getYear(), endDate.getMonth(), endDate.getDay());
    }

    public void setRange(Calendar minDate, Calendar maxDate) {
        TrpCalendar startDate = new TrpCalendar().setTime(minDate.getTime());
        TrpCalendar endDate = new TrpCalendar().setTime(maxDate.getTime());
        if (minDate.compareTo(maxDate) > 0) return;
        updateRange(startDate.getYear(), startDate.getMonth(), startDate.getDay(),
                endDate.getYear(), endDate.getMonth(), endDate.getDay());
    }

    private void updateRange(int minYear, int minYearMonth, int minYearDay,
                             int maxYear, int maxYearMonth, int maxYearDay) {
        ...
        if (!isInRange(mDelegate.mSelectedCalendar)) {
            setCurrentSelected(mDelegate.getMinRangeCalendar());
        }
        mMonthPager.updateRange();
    }

    public void setCurrentSelected(TrpCalendar calendar) {
        mDelegate.mSelectedCalendar = calendar;
...
//        mWeekPager.setVisibility(GONE);
    ...
    public void scrollToDate(TrpCalendar calendar, boolean smoothScroll) {
        ...
        if (smoothScroll) {
            ...
            getMonthViewPager().getLayoutManager().startSmoothScroll(smoothScroller);
        } else {
            ((LinearLayoutManager) getMonthViewPager().getLayoutManager()).scrollToPositionWithOffset(position, 0);
        }
    }
    ...
    public void scrollToCurrent(boolean smoothScroll) {
        TrpCalendar mCurrentSelected = mDelegate.mSelectedCalendar == null ? mDelegate.mSelectedStartRangeCalendar : mDelegate.mSelectedCalendar;
        scrollToDate(mCurrentSelected, smoothScroll);
    }
...
//        mWeekPager.clearMultiSelect();
    ...
    public void setAdditionData(AdditionData additionData) {
        mDelegate.setAdditionData(additionData);
        mMonthPager.getAdapter().notifyDataSetChanged();
    }
...
//        this.mMonthPager.mWeekBar = mWeekBar;
     ...
     */
    public final void setOnCalendarInterceptListener(CalendarView.OnCalendarInterceptListener listener) {
        if (listener == null) {
    ...
    public final void setOnCalendarRangeSelectListener(CalendarView.OnCalendarRangeSelectListener listener) {
     ...
     * @param listener listener
     */
    public final void setOnCalendarMultiSelectListener(CalendarView.OnCalendarMultiSelectListener listener) {
     ...
     * @param listener                 listener
     */
    public void setOnCalendarLongClickListener(CalendarView.OnCalendarLongClickListener listener, boolean preventLongPressedSelect) {
        this.mDelegate.mCalendarLongClickListener = listener;
...
//        mWeekPager.updateWeekStart();
     ...
     * @return 获得月视图
     */
    public com.chungld.uipack.calendar.MonthViewList getMonthViewPager() {
        return mMonthPager;
    }
