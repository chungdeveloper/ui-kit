...
import com.chungld.uipack.calendar.calendar.LunarCalendarVietnamese;
...
public final class LunarCalendar {
    ...
    public static String getLunarText(int year, int month, int day) {
        String termText = com.chungld.uipack.calendar.LunarCalendar.getSolarTerm(year, month, day);
        String solar = com.chungld.uipack.calendar.LunarCalendar.gregorianFestival(month, day);
        ...
        if (!TextUtils.isEmpty(festival))
            return festival;
        return com.chungld.uipack.calendar.LunarCalendar.numToChinese(lunar[1], lunar[2], lunar[3]);
    }
     ...
     */
    public static void setupLunarCalendar(TrpCalendar calendar) {
        ...
        TrpCalendar lunarCalendar = new TrpCalendar();
        calendar.setLunarCalendar(lunarCalendar);
//        int[] lunar = LunarUtil.solarToLunar(year, month, day);

        int[] lunar = LunarCalendarVietnamese.convertSolarToLunarDate(day, month, year,
                (int) TimeUnit.HOURS.convert(java.util.Calendar.getInstance().getTimeZone().getRawOffset(), TimeUnit.MILLISECONDS));

        lunarCalendar.setYear(lunar[2]);
        lunarCalendar.setMonth(lunar[1]);
        lunarCalendar.setDay(lunar[0]);
        calendar.setLeapYear(CalendarUtil.isLeapYear(year));
        if (lunar[0] == 1) {
            calendar.setLeapMonth(lunar[1]);
            lunarCalendar.setLeapMonth(lunar[1]);
        }
        String solarTerm = com.chungld.uipack.calendar.LunarCalendar.getSolarTerm(year, month, day);
        String gregorian = com.chungld.uipack.calendar.LunarCalendar.gregorianFestival(month, day);
        String festival = "";//getTraditionFestival(lunar[0], lunar[1], lunar[2]);
        String lunarText = LunarCalendarVietnamese.getLunarText(lunar);//LunarCalendar.numToChinese(lunar[1], lunar[2], lunar[3]);
...
//        }
     ...
     * @return 获取农历节日
     */
    public static String getLunarText(TrpCalendar calendar) {
